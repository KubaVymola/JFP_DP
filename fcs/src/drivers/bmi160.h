 ...
 * @param hbmi160 Handle for the bmi160 structure - contains handle to I2C and device address
 ...
 */
void bmi160_init(struct bmi160 *hbmi160,
                 I2C_HandleTypeDef *hi2c,
                 uint8_t sd0_pin,
                 uint8_t accel_rate,
                 uint8_t accel_range,
 ...
 */
void bmi160_softreset(struct bmi160 *hbmi160);
/**
 * Called by the bmi160_init function
 */
void bmi160_start_accel(struct bmi160 *hbmi160);
/**
 * Called by the bmi160_init function
 */
void bmi160_start_gyro(struct bmi160 *hbmi160);

/**
 * @return If everything is OK, then the returned value should be OxD1
 */
uint8_t bmi160_get_chip_id(struct bmi160 *hbmi160);
/**
 * @return Returns 1 if chip ID is equal to OxD1
 */
uint8_t bmi160_test_chip_id(struct bmi160 *hbmi160);

/**
 * Sets range and rate to accelerometer and updates the scaling factor.
 * Called by the bmi160_init function.
 */
void bmi160_config_accel(struct bmi160 *hbmi160, uint8_t acc_rate, uint8_t acc_range);
/**
 ...
 */
uint8_t bmi160_get_data_rdy(struct bmi160 *hbmi160);

/**
 * Updates the raw and scaled values in the bmi160 struct for both accel and gyro
*/
void bmi160_update_acc_gyro_data(struct bmi160 *hbmi160);
float bmi160_get_temp_data(struct bmi160 *hbmi160);

void bmi160_configure_interrupts(struct bmi160 *hbmi160);
